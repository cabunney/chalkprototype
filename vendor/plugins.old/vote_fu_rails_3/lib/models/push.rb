class Push < ActiveRecord::Base
  ...
  named_scope :for_pusher,    lambda { |*args| {:conditions => ["pusher_id = ? AND pusher_type = ?", args.first.id, args.first.type.name]} }
  named_scope :for_pushable, lambda { |*args| {:conditions => ["pushable_id = ? AND pushable_type = ?", args.first.id, args.first.type.name]} }
